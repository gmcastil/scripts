...
modelsim_root="${TOOLS_ROOT}/Questa/${modelsim_version}/modelsim_ase"

ok_to_export=0

# Since the specific installation of ModelSim we're using has its own FreeType
# library built we need to let the tool (ultimately the `vish` binary) know
# about it - obviously, if it's not there, something is wrong
if [[ ! -d "${modelsim_root}/lib32" ]]; then
  err "No 32-bit libraries found at ${modelsim_root}/lib32"
  ok_to_export=1
else
  LD_LIBRARY_PATH="${modelsim_root}/lib32:${LD_LIBRARY_PATH}"
fi
...
command -v vsim >/dev/null 2>&1 || \
  { err "vsim not found in PATH"; exit 1; }
